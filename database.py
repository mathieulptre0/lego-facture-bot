...
def get_user_coins(user_id) -> int:
    user_id = int(user_id) # Sécurité pour forcer l'ID en int
    user_data = coins_collection.find_one({"user_id": user_id})
    return user_data["coins"] if user_data else 0

def update_user_coins(user_id, amount: int) -> int:
    user_id = int(user_id) # Sécurité pour forcer l'ID en int
    user_data = coins_collection.find_one({"user_id": user_id})
...
def save_user_coupon(user_id, coupon_code: str, percentage: int):
    user_id = int(user_id)
...
def load_user_coupons(user_id):
    user_id = int(user_id)
    return list(coupons_collection.find({"user_id": user_id}))

def delete_user_coupon(user_id, coupon_code: str):
    user_id = int(user_id)
