...
# Utilisation d'une classe Bot personnalisée pour synchroniser les commandes proprement
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if coins <= 0:
            embed_err = discord.Embed(
                title="Error",
                description=f"You **do not have enough coins** in your account to create a receipt. Please **open a ticket** at <#{TICKET_CHANNEL_ID}> to **order some**!",
        ...
        try:
            ...
            embed_err = discord.Embed(
                title="Error",
                description=f"An error occurred while generating the PDF: **{str(e)}**",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed_err, ephemeral=True)
            return

        pdf_path = os.path.join(r"C:\Users\leazy\Desktop\lego facture bot", "Receipt.pdf")
        
        embed_success = discord.Embed(
            title="<:check:1542642100938477680> Receipt successfully created !",
            description=f"We have **finished creating** your invoice.\n\n<:edit:1542564664192409631> **__Updated information__:**\n\n<:name:1542884240985948240> **Item name:** ```{nom_article}```\n\n<:euro:1542884660105715842> **Item price:** ```{prix_formate}```\n\n<:date:1542886387358105681> **Purchase date:** ```{date_str}```",
            color=0x0058FF
        )
        embed_success.set_footer(text="LEGO® Invoicing System")

        file = discord.File(pdf_path, filename="Receipt.pdf")
        await interaction.response.send_message(embed=embed_success, file=file, ephemeral=True)

class ReceiptView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="Create your receipt", style=discord.ButtonStyle.secondary, emoji="<:receipt:1542877245406978148>", custom_id="create_receipt_button")
    async def create_receipt(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(ReceiptModal())

@bot.tree.command(name="receipt", description="Display the receipt creation panel in the designated channel.")
async def receipt_cmd(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title="<:receipt:1542877245406978148> Welcome to our **server's invoicing system**! :stars: 1542297435664154695",
        description=f"You can **generate your receipt** using the **button below** this message. You will only need to provide a **few details** to **finalize the creation** of your receipt.\n\n:coin: 1542297155660812348 Cost to create a receipt :\n\nGenerating a receipt will **deduct 1 coin** from your **personal balance**. If you **don't have any coins** in your account, you can **open a ticket** at <#{TICKET_CHANNEL_ID}> to **order some**!\n\n:info: 1542297839026053190 Unfortunately, only the **French version** is currently available; our team will **soon** be making **other languages** available to you.",
        color=0x0058FF
    )
