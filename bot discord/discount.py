...
def save_user_coupon(user_id: int, coupon_code: str, percentage: int):
  """Save the coupon in the database under the user's ID."""
  db = load_database()
  ...
  # Ajoute le nouveau coupon à la liste de l'utilisateur
  db[str_user_id].append({"code": coupon_code, "Percentage": percentage})
...
class Discount(commands.Cog):
  ...
  async def discount(
    ...
    # 2. Génération du code et sauvegarde dans la base de données
