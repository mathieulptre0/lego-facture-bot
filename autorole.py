...
from database import db  # On importe directement l'instance db depuis ton database.py
...
class AutoRole(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        # On crée une collection dédiée dans la même base de données
        self.left_roles_collection = db["left_member_roles"] if db is not None else None
    ...
    async def on_member_remove(self, member: discord.Member):
        # Sauvegarder les rôles du membre avant qu'il ne quitte (on exclut @everyone et les rôles gérés par des bots/intégrations)
        if self.left_roles_collection is not None:
