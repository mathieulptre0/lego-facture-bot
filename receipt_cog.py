...
# Importe la fonction de génération depuis receipt.py
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if coins <= 0:
            embed_err = discord.Embed(
                description="❌ You **don't have enough coins** in your account to generate a receipt. Please open a ticket to order some!",
        ...
        try:
            ...
            return await interaction.response.send_message(
                "❌ **Invalid price format.** Please enter a valid number (e.g., `5.99` or `1`).", 
                ephemeral=True
            )
        ...
        try:
            ...
            return await interaction.response.send_message(
                "❌ **Invalid date format.** Please use: `JJ/MM/AAAA HH:MM:SS` (e.g., `24/08/2026 19:35:15`).", 
                ephemeral=True
            )
        ...
        embed_succes = discord.Embed(
            ...
            description=(
                ...
                f"<:name:1542884240985948240> **Item name:** ```{self.item_name.value.strip()}```\n"
                f"<:euro:1542884660105715842> **Item price:** ```{prix_str_formate}```\n"
                f"<:date:1542886387358105681> **Purchase date:** ```{texte_date_valeur}```"
            ),
            color=0x0058ff
        )
        ...
        class DownloadView(ui.View):
            ...
            @ui.button(label="Download the receipt", emoji="<:download:1542886821279563918>", style=discord.ButtonStyle.secondary)
            async def download_btn(self, btn_interaction: discord.Interaction, button: ui.Button):
                await btn_interaction.response.send_message(
                    "📥 Here is your PDF receipt file:", file=discord.File(self.file_path), ephemeral=True
                )

        view = DownloadView(pdf_path)
        await interaction.response.send_message(embed=embed_succes, view=view, file=file_to_send, ephemeral=True)
...
class PersistentReceiptView(ui.View):
    ...
    @ui.button(label="Create your receipt", emoji="<:receipt:1542877245406978148>", style=discord.ButtonStyle.secondary, custom_id="persistent_create_receipt_btn")
    async def create_receipt_button(self, interaction: discord.Interaction, button: ui.Button):
...
class ReceiptCog(commands.Cog):
    ...
    async def cmd_receipt(self, ctx):
        role = ctx.guild.get_role(ROLE_AUTORISE_ID)
        ...
        if not salon:
            return await ctx.send("❌ Erreur : Le salon configuré pour l'envoi est introuvable.", delete_after=5)
        ...
        embed_public = discord.Embed(
            ...
            description=(
                ...
                f"If you **don't have any coins** in your account, you can **open a ticket** at <#{SALON_TICKET_SUPPORT_ID}> to **order some**!\n\n"
